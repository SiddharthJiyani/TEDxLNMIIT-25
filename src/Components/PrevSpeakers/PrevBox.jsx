...
const PrevBox = ({ imgSrc, altText, name, description, url }) => {
  return (
    <div className="hover:-translate-y-3 transition-transform duration-300">
      <div className="m-4 group relative border-4 border-[#141414] rounded-2xl w-[80%] md:w-[250px] h-[80%] md:h-[290px] p-1 bg-[#141414] overflow-hidden">
        {/* Image always shown */}
        <img
          className={`rounded-2xl transition-opacity duration-300 w-full h-full object-cover ${description || url ? 'group-hover:opacity-0' : ''}`}
          src={imgSrc}
          alt={altText}
        />
